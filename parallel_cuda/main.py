...
def schwefel(x):
    x = cp.asarray(x)
    n = x.size
    return 418.9829 * n - cp.sum(x * cp.sin(cp.sqrt(cp.abs(x))))


def rosenbrock(x):

    x = cp.asarray(x)
    return cp.sum(100.0 * (x[1:] - x[:-1] ** 2) ** 2 + (1.0 - x[:-1]) ** 2)
...
def inicjalizuj_roj(objective, n_dim, bounds, swarm_size, random_state=None):
    ...
    pbest_values = cp.apply_along_axis(objective, 1, positions)
...
def wykonaj_iteracje(
    ...
    values = cp.apply_along_axis(objective, 1, positions)
    ...
    best_history.append(cp.asnumpy(gbest_value))
